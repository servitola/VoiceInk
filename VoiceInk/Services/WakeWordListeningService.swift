...
class WakeWordListeningService: NSObject, ObservableObject {
    ...
    private func loadSettings() {
        ...
        microphoneUID = UserDefaults.standard.string(forKey: "wakeWordMicrophoneUID") ?? ""
    ...
    func configureMicrophone(uid: String) {
        self.microphoneUID = uid

        UserDefaults.standard.set(uid, forKey: "wakeWordMicrophoneUID")
    ...
    /// Resolves the Core Audio device the detector should use.
    /// Prefers an explicitly chosen device, then falls back to the app's current recording device.
    private func resolveInputDeviceID() -> AudioDeviceID? {
        let manager = AudioDeviceManager.shared

        if !microphoneUID.isEmpty,
            let device = manager.availableDevices.first(where: { $0.uid == microphoneUID }) {
            return device.id
        }
    ...
    private func startRecognition() async throws {
        ...
        if let deviceID = resolveInputDeviceID() {
            do {
                ...
                logger.notice("Wake word listening on input device \(deviceID, privacy: .public)")
            } catch {
                logger.error("Failed to set wake word input device: \(error.localizedDescription)")
            }
        } else {
            logger.notice("No specific input device resolved; using system default for wake word")
        }
