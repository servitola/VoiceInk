...
# `-legacy` forces RC2/3DES + SHA1 which Security.framework supports.
openssl pkcs12 -export -legacy -out "$P12" -inkey "$KEY" -in "$CRT" -password "pass:$P12_PASS" >/dev/null 2>&1
